...
class SwiftNetResNet(nn.Module):
    def __init__(self, block, layers, num_features=(128, 128, 128), k_up=3, efficient=True, use_bn=True,
        ...
        if self.build_decoder:
            upsamples = []
            upsamples += [_Upsample(num_features[2], self.inplanes_list[0], num_features[2], use_bn=self.use_bn, k=k_up)]   #adding
    ...
    def forward_up(self, features):
        ...
        features = features[::-1]
        x = features[0]
        multi_scale_features = []               #adding
        for skip, up in zip(features[1:], self.upsample):
            x = up(x, skip)
            multi_scale_features.append(x)      #adding
        # return x
        # return multi_scale_features[2], multi_scale_features[:-1]          #adding, just get feature scale 1:2 and 1:8 -> 1:2
        return multi_scale_features[2], multi_scale_features[1:]          #adding, just get feature scale 1:2 and 1:4 -> 1:1  
    def forward(self, image):
        # feature, multi_scale_features = self.forward_up(self.forward_down(image))
        return self.forward_up(self.forward_down(image))
        # return feature, multi_scale_features
