...
from pasco.models.layer.modules import ASPP, Upsample
from pasco.models.layer.deformable_transformer import DeformableTransformerLayer, TransformerLayer
from pasco.models.layer.pos_embed import LearnableSqueezePositionalEncoding
from pasco.models.layer.deform_squeeze_attn import DeformableSqueezeAttention
# from ..projections import VoxelProposalLayer
from pasco.utils.utils import (cumprod, flatten_fov_from_voxels, flatten_multi_scale_feats, generate_grid,
...
class SymphoniesLayer(nn.Module):
    ...
    def forward(self,
                scene_embed_dense,          #[bs, X*Y*Z, C]
                scene_embed_fov,            #[bs, N, C]  # N is the number of voxels in the FOV
                inst_queries,               #[bs, N, C]
                feats,
                scene_pos=None,             #[bs, N, hidden_dim = C] 
                inst_pos=None,              #[bs, N, C] 
                ref_2d=None,                #[bs, N, level, 2]
                ref_3d=None,
                ref_vox=None,
                scene_size=None,
                fov_mask=None):
        # scene_embed_fov = flatten_fov_from_voxels(scene_embed, fov_mask)          #scene_embed_fov shape [1, N, C]
        # scene_pos_fov = flatten_fov_from_voxels(scene_pos,
        #                                         fov_mask) if scene_pos is not None else None
        # scene_embed_flatten, scene_shape = flatten_multi_scale_feats([scene_embed_dense]) #scene_embed_flatten shape [1, XYZ, C], scene_shape shape [X, Y, Z]
        scene_level_index = get_level_start_index(scene_size)
        ...
        scene_embed = self.scene_query_cross_attn(scene_embed_fov, inst_queries, inst_queries,
                                                      scene_pos, inst_pos)
...
#         self.scene_shape = scene_shape                                #shape [X, Y, Z]
...
#         voxel_grid = generate_grid(scene_shape, normalize=True)       #voxel_grid [3, X, Y, Z]
...
#         ref_vox = nchw_to_nlc(self.voxel_grid.unsqueeze(0)).unsqueeze(2)              #self.voxel_grid [3, X, Y, Z]
                                                                                        #ref_vox [1, XYZ, 1, 3]

#         scene_embed = self.scene_embed.weight.repeat(bs, 1, 1)                                
#         scene_pos = self.scene_pos().repeat(bs, 1, 1)
#         scene_embed = self.voxel_proposal(scene_embed, feats, scene_pos, vol_pts, ref_pix)        #scene_embed [1, C, X, Y, Z]
#         scene_pos = nlc_to_nchw(scene_pos, self.scene_shape)                                      #scene_pos [1, C, X, Y, Z]
